...
int  WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_  LPSTR lpszCmdParam, _In_  int nCmdShow)
{
	...
	//--- 크기 변경 가능 (기존 (1024, 768)) 150,10,1200,900 / 150 10 800 800
	hWnd = CreateWindow(lpszClass, lpszWindowName, WS_OVERLAPPEDWINDOW, 150, 10, 1200, 900, NULL, (HMENU)NULL, hInstance, NULL);
...
void eatFood(ONE* one, FOOD food[50], ONE virus[20])
{
	for (int i = 0; i < FoodCount; i++)
	{
		...
		{
			one->radius++;
			//	if(one->PX - 0.05>= 2.0) one->PX -= 0.05;
			//	if (one->PY - 0.05>= 2.0) one->PY -= 0.05;
	...
	for (int i = 0; i < FoodCount; i++)
	{
		for (int k = 0; k < Viruscount; k++)
		{
			...
			{
				virus[k].radius++; // 3
				//if (virus[k].PX - 0.50 >= 2.0)
				//	virus[k].PX -= 0.05;
			//	if (virus[k].PY - 0.50 >= 2.0)
				//	virus[k].PY -= 0.05;
...
void createVirus(HWND hwnd, ONE virus[20], int* countVirus)
{
	...
	virus[*countVirus].PX = 1.5;
	virus[*countVirus].PY = 1.5;
	(*countVirus)++;
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		{
			view.left = one.pivot.x - 300 - one.radius;
			view.top = one.pivot.y - 300 - one.radius;
